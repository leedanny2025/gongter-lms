...
export default function WeeklyNoticePage() {
  ...
  const rows = state.students.filter(s => group === '전체' || s.classGroup === group).sort((a,b) => a.name.localeCompare(b.name,'ko')).map(student => ({ student, progress: getWeeklyProgress(state, student, week), monthlyProgress: getMonthlyProgress(state, student, month) }));
  ...
  return <div className={styles.page}>
    ...
    <article className={styles.paper}>
      ...
      <table className={styles.table}>
        ...
        <thead><tr><th scope="col">이름</th><th scope="col">출석</th><th scope="col">숙제</th><th scope="col">시험</th><th scope="col">태도</th><th scope="col">{monthly ? '지급 달러' : '달러 성취'}</th></tr></thead>
        <tbody>{rows.map(({student,progress:p,monthlyProgress}) => { const stats = monthly ? monthlyProgress : p; return <tr key={student.id}>
      ...
      <footer className={styles.footer}><strong>꼭 확인해 주세요</strong><p>숫자는 완료한 일수 / {periodLabel} 수업 예정 일수입니다.<br />숙제는 완료·승인, 시험은 점수 확정 기록을 반영합니다.<br />태도 점수는 해당 기간의 쉐도잉·학습 태도·기본 태도 합계입니다.<br />{monthly ? '월간은 매월 1일~말일 기준이며, 달러는 한국 시간 지급일 기준 실제 지급 합계입니다.' : '달러는 태도 보너스를 포함한 예상 지급액이며 실제 지급액과 다를 수 있습니다.'}</p><div>금요일 자정에 새 주 시작 · 보유 달러와 지난 기록 보존</div></footer>
    </article>
